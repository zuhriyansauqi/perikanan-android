...
class FisheryRepositoryTest_UpdateFishery : FisheryRepositoryTestFixture() {
  @Before
  fun setUp() {
  ...
  fun `given api result is correct when deleteFishery called should return domain model correctly`() =
    testScope.runTest {
      coEvery { api.updateFishery(any()) } returns Any()
      every { dispatchers.io } returns testDispatcher

      repository.updateFishery(uuid = "1234", params = emptyMap())

      coVerify { safeApiCall<Any>(any(), any()) }
      coVerify { api.updateFishery(any()) }
    }
